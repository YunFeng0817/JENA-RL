...
/**
 * Machinary. This code implements the connectiveness assumed by execution based
 * on substitution (index joins). i.e. if <code>{ ?x :p ?v . ?x :q ?w }</code>
...
public abstract class ReorderTransformationSubstitution implements ReorderTransformation {
    ...
    protected ReorderProc reorder(List<Triple> triples, List<PatternTriple> components) {
        ...
        if (triples.size() != idx)
            throw new ARQException(String.format(
                    "Inconsistency: number of triples (%d) does not equal to number of indexes processed (%d)",
                    triples.size(), idx));
    ...
    /**
     * Return the index of the first, least triple; optionally accumulate all
     * indexes of the same least weight
     */
    ...
    /**
     * What to do if the {@link weight} comes back as "not found". Choices are: ZERO
     * Assume the weight is zero (the rules were complete over the data so this is a
     * pattern that will not match the data. LAST Place after all explicitly
     * weighted triple patterns FIRST Place before all explicitly weighted triple
     * patterns NUMERIC Use value returned by {@link defaultWeight} The default,
     * default choice is LAST.
     */
