 ...
 * for basic graph patterns and filtered basic graph patterns (currently).
 * 
...
public class OpExecutorTDB1 extends OpExecutor {
    ...
    protected QueryIterator execute(OpFilter opFilter, QueryIterator input) {
        ...
        if (opFilter.getSubOp() instanceof OpQuadPattern) {
            ...
            return optimizeExecuteQuads(ds, input, quadPattern.getGraphNode(), quadPattern.getBasicPattern(),
    ...
    protected QueryIterator execute(OpQuadPattern quadPattern, QueryIterator input) {
        ...
        // throw new InternalErrorException("Not a TDB backed dataset in quad pattern
        // execution") ;
    ...
    /**
     * Execute a BGP (and filters) on a TDB graph, which may be in default storage
     * or it may be a named graph
     */
    ...
    private static QueryIterator optimizeExecuteQuads(DatasetGraphTDB ds, QueryIterator input, Node gn,
            BasicPattern bgp, ExprList exprs, ExecutionContext execCxt) {
        if (!input.hasNext())
            return input;
    ...
    public static Node decideGraphNode(Node gn, ExecutionContext execCxt) {
        ...
        // Quad.defaultGraphNodeGenerated -- the internal marker node used for the quad
        // form of queries.
    ...
    // Is this a query against the real default graph in the storage (in a 3-tuple
    // table).
